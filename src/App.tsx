...
const App = () => {
  ...
  const closeModal = (e: any) => {
    if (
      e.path &&
      e?.path[1] !== modalRef.current &&
      !e.path.includes(viewSwitch.current)
    ) {
  ...
  const refresh = () => {
    ...
    if (!context.viewroot && !localStorage.getItem("loggedUser")) {
      context.wallet.dispatchNotification(
        "You should login first for this action!!"
      )
    }
  }
  ...
  return (
    <div className="App">
      <div className="header" style={{ padding: "0rem 10%" }}>
                  ...
                  .map((node: any, index: number) => {
                    return (
                      <div
                        ...
                        style={{
                          color:
                            index === context.wallet.networkIndex
                              ? "#003fe3"
                              : "inherit",
                        }}
                        className="networkentry"
                        onClick={() => switchNetwork(index)}
                      >
                        {node.name}
                      </div>
                    )
        ...
        >
          <div className="refresh">
            ...
            {adminList.includes(context.github.loggedUser) && (
              <Button
                size="small"
          ...
          <div
            style={{ cursor: "pointer", margin: "0px 12rem" }}
            onClick={() => history.push("/")}
          >
            <img src={Logo} title="Return to home page" alt="Filecoin" />
          </div>
        ...
        <div className="headerLeftRight">
          ...
          >
              ...
              <div className="accountselectholder" ref={viewSwitch}>
                ...
                >
                  <div>
                    ...
                    <div>
                      {context.viewroot ? "Rootkey Holder" : "Approved Notary"}
                    </div>
                  </div>
                    ...
                    <div className="accountentry">
                      <div>
                        <div className="datacapdata">
                          <span className="datacap">
                            Datacap:{" "}
                            {bytesToiB(
                              getVerifierAmount(context.wallet.multisigID)
                            )}
                          </span>
                        ...
                        <div className="accountdata">
                          <span className="accountaddress">
                            {context.wallet.multisigAddress.length > 20
                              ? addressFilter(context.wallet.multisigAddress)
                              : context.wallet.multisigAddress}
                          </span>
                          <span className="copyaddress">
                            <SVG.CopyAndPaste height="15px" />
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="headertitles" style={{ paddingTop: "10px" }}>
                  Account addresses
                </div>
                  ...
                  {context.wallet.accounts.map((account, index) => {
                    return (
                      <div
                        ...
                        style={{
                          backgroundColor:
                            index === context.wallet.walletIndex
                              ? "#C7C7C7"
                              : "inherit",
                        }}
                      >
                        <div>
                          <div
                            className="datacapdata"
                            onClick={() => switchAccount(index)}
                          >
                            {!context.viewroot ? (
                              <span className="datacap">
                                Datacap: {bytesToiB(getVerifierAmount(account))}
                              </span>
                            ) : (
                              <span className="datacap"></span>
                            )}
                ...
                {context.wallet.accounts.length > 5 && (
            ...
            >
              {context.viewroot ? "Rootkey Holder ID" : "Approved Notary ID"}
            </div>
            <div
              onClick={(e) => {
                e.stopPropagation()
            ...
            >
              {addressFilter(context.wallet.activeAccount)},{" "}
              {context.wallet.multisig && !context.viewroot
                ? context.wallet.multisigAddress.length > 20
                  ? addressFilter(context.wallet.multisigAddress)
                  : context.wallet.multisigAddress
            ...
            {context.github.githubLogged ? (
              <div className="avatarContainer">
                <img
                  ...
                  src={
                    localStorage.getItem("avatar") || context.github.avatarUrl
                  }
              ...
              <div className="avatarContainer">
                <img
                  className="avatarImage"
                  src="https://avatars.githubusercontent.com/u/72555788?v=4"
                  alt="user_profile_photo"
                />
              </div>
            )}
      ...
      {context.wallet.isLoading || !context.wallet.isLogged ? (
