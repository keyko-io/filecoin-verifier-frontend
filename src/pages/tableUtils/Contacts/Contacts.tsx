        ...
        <div style={{ display: 'flex', flexDirection: 'column', margin: '6px 0' }}>
            <div className='contacts-box'>
                ...
                <CopyToClipboard text={slack} onCopy={() => setIsCopySlack(true)}>
                    <span className={isCopySlack ? 'active' : ''} style={{ cursor: 'pointer' }}>
                        {isCopySlack ? <CheckRoundedIcon /> : <ContentCopyRoundedIcon />}
                    </span>
                </CopyToClipboard>
            </div>
            {href === undefined ? null : href.includes('@') ? (
                <div className='contacts-box'>
                    {isCopyMail ? href : 'Mail'}
                    <CopyToClipboard text={href} onCopy={() => setIsCopyMail(true)}>
                        <span className={isCopyMail ? 'active' : ''} style={{ cursor: 'pointer' }}>
                            {isCopyMail ? <CheckRoundedIcon /> : <ContentCopyRoundedIcon />}
                        </span>
                    </CopyToClipboard>
                </div>
            ) : (
