...
const LogExplorer = () => {
    ...
    const selectIssueNumber = async (issueNumberId?: string) => {
        ...
        try {
            ...
            const res = issueNumberId
                ? await fetchLogs(issueNumberId)
                : await fetchLogs(issueNumber)
    ...
    const formatItems = (items: LogsItem[]) => {
        ...
        return newItemsArray.sort(
            (a, b) => new Date(b[sortBy]).valueOf() - new Date(a[sortBy]).valueOf(),
        )
    }
    ...
    return (
        <div className='landing'>
                ...
                <div className='wrapperverifiers'>
                    <div className='tableselects'>
                        <div className='tabletitle'>
                            <div className='title'>
                                Select issue number to show corresponding logs <br />{' '}
                                <span style={{ fontWeight: 'normal', fontSize: '14px' }}>
                                    {' '}
                                    - type 0 to see general logs
                                </span>{' '}
                            </div>
                            ...
                            <form style={{ display: 'flex' }}>
                                ...
                                <Button
                                    ...
                                    color='primary'
                                    style={{
                                        backgroundColor: isLogsLoading ? '#3f51b5' : '',
                                        borderLeft: issueNumber === '' ? '1px solid #111212' : '',
                            ...
                            <table>
                                ...
                                <tbody>
                                    {logs
                                        .filter(
                                            (item) =>
                                                item.message.match(new RegExp(searchText, 'gi')) ||
                                                item.repo.match(new RegExp(searchText, 'gi')),
                                        )
                                        .filter((item) =>
                                            date
                                                ? new Date(item.dateTimestamp).toLocaleDateString(
                                                      'fr-CA',
                                                      {
                                                          year: 'numeric',
                                                          month: '2-digit',
                                        ...
                                        {!isLogsLoading ? (
                                            <td colSpan={7}>
                                                {logs.length ? (
                                                    ...
                                                    <div>Select issue number and search</div>
                                                )}
                                            </td>
                                        ) : (
                                            <td colSpan={7}>{<div>Loading...</div>}</td>
                                        )}
                                    </tr>
                                </tbody>
                            </table>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '30px' }}>
                <div className='gotoTopLogs' onClick={onTop}>
