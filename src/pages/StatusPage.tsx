...
const StatusPage = () => {
    ...
    const checkLDNStatus = async () => {
        try {
            ...
            const statusIssue = await github?.githubOctoGeneric?.octokit?.rest?.issues.get({
                owner,
                repo,
    ...
    const checkSSAStatus = async () => {
        ...
        try {
            ...
            const is_Ssa_Healhty =
                moment.duration(moment(new Date()).diff(moment(last_SSA))).asHours() < 3.2
    ...
    const checkHealthStatus = (isLoading: boolean, botStatus: boolean | null) => {
        if (isLoading) {
            ...
            if (botStatus) {
                ...
                return <DoNotDisturbOnIcon sx={{ color: 'red' }} />
            }
        }
    }
            ...
            <Box sx={{ width: '60rem' }}>
                <Paper elevation={4}>
                    <Stack sx={{ p: '1rem' }} direction='row' justifyContent='space-between'>
                        <Typography variant='h6'>Current status by service</Typography>
                    ...
                    <Stack>
                        <Stack sx={{ p: '1rem' }} direction='row' justifyContent='space-between'>
                            <Typography variant='body1'>SSA Bot</Typography>
                            {checkHealthStatus(isSSALoading, isSSAHealhty)}
                        </Stack>
                        <Divider />
                        <Stack sx={{ p: '1rem' }} direction='row' justifyContent='space-between'>
                            <Typography variant='body1'>LDN Validation Bot</Typography>
                            {checkHealthStatus(isLDNLoading, isLDNBotHealthy)}
                        </Stack>
                    </Stack>
                </Paper>
            </Box>
            <Stack direction='row' justifyContent='center' alignItems='center' marginTop={10}>
                {!isSSALoading && (
                    <Alert severity='info'>
                        <Typography variant='body1'>
                            The SSA bot runs every 3 hours and triggers new datacap requests if
                            needed.
                        </Typography>
                        <Typography variant='body1'>
                            Last time active was: <span>{moment(lastSSATime).fromNow()}</span>
                        </Typography>
                        <Typography variant='body1'>
                            LDN Validation Bot is always active and listens to events happening on
                            the repository.
                        </Typography>
                        {/* <Typography variant="body1">The last time SSA bot ran: <span>{moment(lastSSATime).fromNow()}</span></Typography> */}
                        <Typography variant='body1'>
                            Both bots update this issue:
