...
const LDNBotHealthCheck = async (gh: any): Promise<boolean> => {
  try {
    ...
    await testAsync();
    const issueAfterChange = await gh?.githubOcto?.rest?.issues.get({
      owner: "keyko-io",
      repo: "filecoin-large-clients-onboarding",
      issue_number: 1407,
    });
...
const StatusPage = () => {
  ...
  const [isLDNBotHealthy, setIsLDNBotHealthy] = useState<boolean>(false);
        ...
        <Paper elevation={4}>
          ...
          <Stack>
            ...
            <Stack
              ...
              justifyContent="space-between"
            >
              <Typography variant="body1">LDN BOT</Typography>
              {isLDNBotHealthy ? (
                <CheckCircleIcon sx={{ color: "green" }} />
              ) : isLoading ? (
                "Checking.."
