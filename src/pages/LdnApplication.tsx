...
class LdnApplication extends Component<LdnApplicationProps> {
    ...
    async fetchIssuesAndSelectView() {
        ...
        try {
            const issues = (
                await this.context.github.githubOcto.issues.list({
                    filter: 'created',
    ...
    validateDatacapAndAddress(coreInfo: any, name: any, value: any, elementToUpdateIndex: any) {
        ...
        if (coreInfo[elementToUpdateIndex].value !== '') {
            ...
            if (
                coreInfo[elementToUpdateIndex].name === 'address' &&
                coreInfo[elementToUpdateIndex].value.substring(0, 2) !== 'f1' &&
                coreInfo[elementToUpdateIndex].value.substring(0, 2) !== 'f3'
            ) {
                hasError = true
    ...
    updateCoreInfoContent(name: any, value: string, isMeasure?: boolean) {
        ...
        if (coreInfo[elementToUpdateIndex]) {
            if (
                isMeasure ||
                coreInfo[elementToUpdateIndex].type === 'number' ||
                coreInfo[elementToUpdateIndex].name === 'address'
            )
                hasError = this.validateDatacapAndAddress(
                    coreInfo,
                    name,
                    value,
    ...
    addressListIncludesAddressInput(address: any) {
        if (this.state.addressList.includes(address)) {
            alert(
                'error: You did use already this address for another issue. Please, use another one!',
            )
            return true
        }
        return false
        ...
        try {
            ...
            if (this.state.stepViewFour === 2) {
                await this.context.github.githubOcto.issues.update({
                    ...
                    issue_number: this.state.issueNumber,
                    body: temp,
                    labels: this.isIssueCompleted()
                        ? [labelsIssueCreation.ISSUE_COMPLETED]
                        : [labelsIssueCreation.WIP_ISSUE],
                })
    ...
    render() {
        return (
            <div className={this.state.view === 4 ? 'viewFourContainer' : 'formContainer'}>
                ...
                <div
                    style={{ maxWidth: this.state.view === 5 ? '30%' : '' }}
                    className={guidelines[this.state.view].className}
                >
                    ...
                    {this.state.view === 5 && (
                        <div className='guideText'>
                            <a
                                ...
                                href={
                                    guidelines[this.state.view].link + '/' + this.state.issueNumber
                                }
                            >
                                {' '}
                                repo
                    ...
                    <div className='guideText'>
                        Find more details{' '}
                        <a target='_blank' rel='noopener noreferrer' href={URL_README}>
                            here
                        </a>
                    </div>
                    {this.state.view === 4 && (
                        <div className='guideText'>
                            <small>Step {this.state.stepViewFour + 2} of 4</small>
                            <br></br>
                            <small>
                                These questions can be answered here or by editing the GitHub Issue
                        ...
                        >
                            {steps.map((item: any, index: any) => (
                                <Link
                                    ...
                                    onClick={() =>
                                        this.setState({
                                            view: item === 'Main Info' ? 3 : 4,
                                            stepViewFour: item === 'Main Info' ? 0 : index - 1,
                                        })
                                    }
                                >
                                    {item}
                            ...
                            <LoginGithub
                                ...
                                onSuccess={async (response: { code: string }) => {
                                    try {
                                        ...
                                        this.setState({
                                            loggedUser: this.context.github.loggedUser,
                                        })
                ...
                {this.state.view === 1 && (
                    <div className='content'>
                        <div className='titleIssueBox'>
                            Select an issue to continue editing or{' '}
                            <a
                                style={{ cursor: 'pointer', color: '#0091ff' }}
                                onClick={() => this.setState({ view: 3, isNewIssue: true })}
                            >
                                create a new issue
                            </a>
                        </div>
                    ...
                    <div className='content'>
                                    ...
                                    {item.type === 'number' ? (
                                        <div className='select-slot'>
                                            <TextField
                                                ...
                                                type='number'
                                                helperText={
                                                    this.state.coreInfo[index].error
                                                        ? this.state.coreInfo[index].errorMessage
                                                        : ''
                                                }
                                        ...
                                        <>
                                            ...
                                            <Select
                                                ...
                                                name={item.name}
                                                value={
                                                    this.state.coreInfo[index].value ||
                                                    'Select Region'
                                                }
                                                onChange={(e: any) => this.handleChange(e)}
                                            >
                                                {regionOptions.map((item: any, index: any) => (
                        ...
                        <div className='otherInfoButtonContainer'>
                            ...
                            <Button
                                ...
                                variant='contained'
                                color='primary'
                                onClick={() => this.createUpdateIssue()}
                            >
                        ...
                        <div className='otherInfoContainer'>
                            <div className='otherInfoHeader'>
                                <h1>
                                    {
                                        otherInfo.find(
                                            (item: any) => item.step === this.state.stepViewFour,
                                        )?.category
                                    }
                                </h1>
                            </div>
                                ...
                                (item: any, index: any) =>
                                    this.state.stepViewFour === item.step && (
                                        <div key={index} className={'otherInfoDiv'}>
                                            <label className='otherInfoLabel'>
                                                {item.description}
                                            </label>
                                            <textarea
                                                onChange={(e) =>
                                                    this.fillTextAreas(item.name, e.target.value)
                                                }
                                            >
                                                {item.value}
                                            </textarea>
                            ...
                            {!this.state.stepViewFour ? (
                                <div className='otherInfoButtonContainer'>
                                    <Button
                                        variant='contained'
                                        color='primary'
                                        onClick={() => this.updateIssueAndContinue()}
                                    >
                                        Save and Continue
                                    </Button>
                                </div>
                                ...
                                this.state.stepViewFour === 2 && (
                                    <div className='otherInfoButtonContainer'>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => this.updateIssueAndContinue()}
                                        >
                                            {otherInfo.some((item) => item.value === '')
                                                ? 'Save Draft'
                                                : 'Submit Issue'}
                                        </Button>
                                    </div>
