        ...
        this.state = {
            ...
            postLogs: async (
                ...
                try {
                    ...
                    const res = (
                        await fetch('https://cbqluey8wa.execute-api.us-east-1.amazonaws.com/dev', {
            ...
            // @ts-ignore
            formatLargeRequestData: async (requests: ParseLargeRequestData[]) => {
                if (!requests) return []
            ...
            getLargeRequestSearchInputData: async () => {
                if (!this?.props?.github || !this?.props?.github?.githubLogged) return []
                const allGHIssues = await this?.props?.github?.fetchGithubIssues(
            ...
            getLDNIssuesAndTransactions: async () => {
                    ...
                    for (let i = cmtsLength - 1; i >= 0; i--) {
                        ...
                        if (commentParsed.correct) {
                            const issueInMsig = issuesByMsig.find(
                                (item: any) => item.multisigAddress === commentParsed.notaryAddress,
                            )
                            if (issueInMsig) {
                ...
                for (const msigGroup of issuesByClientAddress) {
                    const txsByCientList: TxsByClientAddress[] = txsGroupedByClientAddress.filter(
                    ...
                    for (const [k, v] of Object.entries(txsByCientList[0].txsByClientAddress)) {
                        console.log(k, v)
                    ...
                    for (const [k, v] of Object.entries(msigGroup.byClients)) {
            ...
            getNodeData: async (address, clientAddress) => {
                const pendingTxs = await this.props.wallet.api.pendingTransactions(address)
                ...
                if (pendingFiltered?.length > 0) {
                    ...
                    const signerAddress = await this.props.wallet.api.actorKey(si)
            ...
            loadVerifierAndPendingRequests: async () => {
                ...
                try {
                    ...
                    const verifierAndPendingRequests = requestsAndComments.map((r: any) => {
                        const datacap = r.tx
            ...
            createRequest: async (data: any) => {
                try {
                    const user = (await this.props.github.githubOcto.users.getAuthenticated()).data
                        .login
            ...
            selectNotaryRequest: (selectedNotaryItems: any) => {
                const selectedNotaries = selectedNotaryItems.map((item: any) => item.id)
            ...
            verifyWalletAddress: async () => {
                try {
                    ...
                    const msgCid = await this.props.wallet.api.methods.sendTx(
                        ...
                        this.props.wallet,
                        this.props.wallet.api.methods.encodeSend(config.secretRecieverAddress),
                    )
                    // if (msgCid) {
                    if (msgCid['/']) {
                        ...
                        for (const issue of rawIssues) {
                            // parse each issue
                            const parsedNotaryAddress = notaryParser.parseNotaryAddress(issue.body)
            ...
            checkVerifyWallet: async () => {
                try {
                    ...
                    for (const issue of rawIssues) {
                        // parse each issue
                        const parsedNotaryAddress = notaryParser.parseNotaryAddress(issue.body)
                        const address = parsedNotaryAddress ? parsedNotaryAddress.split(' ')[0] : ''
