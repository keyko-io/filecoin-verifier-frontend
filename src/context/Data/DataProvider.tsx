...
> {
    constructor(props: DataProviderProps) {
        ...
        this.state = {
            ...
            //@ts-ignore
            testGetLDNIssuesAndTransactions: async (
                rawLargeIssuesAll: any[]
            ) => {
                //GETTING ISSUES
                const rawLargeIssues = rawLargeIssuesAll.filter(
                    (item: any) =>
            ...
            getReadyToSignIssues: async () => {
                try {
                    ...
                    return rawLargeIssuesAll;
                } catch (error) {
                    console.log(error);
                }
            },
            testLoadLarge: async (page = 1) => {
                try {
                    this.setState({ ldnRequestsLoading: true });
                    const rawLargeIssuesAll =
                        await this.state.getReadyToSignIssues();
                    const paginate = rawLargeIssuesAll.slice(
                        (page - 1) * 10,
                        page * 10
                    );
                    console.log("paginate", paginate)
                    const ldnIssueTxs =
                        await this.state.testGetLDNIssuesAndTransactions(
                            paginate
                        );
                    const txsIssueGitHub =
                        ldnIssueTxs.filteredTxsIssue;
                    ...
                    this.setState({
                        largeClientRequests: largeissues.map(
                            (i: any) => i.value
                        ),
                        ldnRequestsLoading: false,
                    });
