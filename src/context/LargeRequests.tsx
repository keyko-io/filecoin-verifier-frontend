...
export const LargeRequestsContext = createContext({} as LargeRequestsState)

export const useLargeRequestsContext = () => useContext(LargeRequestsContext)
...
export default function LargeRequestsProvider({ children }: any) {
    ...
    useEffect(() => {
        const handler = async () => {
            const data = await context.getLargeRequestSearchInputData()
            setData(data)
        }
        handler()
    }, [context])

    const areRequestsSignable = async (requests: LargeRequestData[]): Promise<boolean> => {
                    ...
                    new Promise<boolean>(async (resolve, reject) => {
                        try {
                            ...
                            // if (approvals >=    1)  //TODO manage the case when there is more than 1 request
                                ...
                                resolve(isMultisigIncludesCurrentSigner && approverIsNotProposer)
                            } else {
                                resolve(false)
                            }
                        } catch (error) {
    ...
    return (
        <section className='section'>
            <LargeRequestsContext.Provider value={IState}>{children}</LargeRequestsContext.Provider>
        </section>
    )
}
