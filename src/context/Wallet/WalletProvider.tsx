      ...
      if (options.multisig) {
        try {
          ...
          for (let index = 0; index < multisigInfo.signers.length; index++) {
            const actor = await wallet.api.actorKey(
              multisigInfo.signers[index]
            );
          ...
          if (index !== -1) {
            ...
            this.state.dispatchNotification(
              "Multisig address not found in wallet"
            );
            return false;
          }
        } catch (e: any) {
