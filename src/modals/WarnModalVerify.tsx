...
const WarnModalVerify = (props: ModalProps) => {
  ...
  useEffect(() => {
    ...
    if (props.origin === "Notary") {
      for (const request of props.clientRequests) {
        console.log(request, "xx1122")
        if (
          props.selectedClientRequests.includes(
            request.issue_number || request.number
          )
        ) {
          requestToShow.push({
      ...
      setMessage(
        "You are about to send a message to assign DataCap to the following addresses:"
      )
      ...
      for (const request of props.clientRequests) {
        if (props.selectedClientRequests.includes(request.id)) {
          requestToShow.push({
            ...
            datacap: request.datacaps.map((datacap: any) =>
              anyToBytes(datacap) === 0 ? "Notary will be removed" : datacap
            ),
          })
        }
      }
  ...
  const renderArray = (arrayValues: any[]) => {
    return (
      <>
        {arrayValues.map((value: any, i: any) => (
          <div key={i} className="elemvalue">
            {value}
          </div>
        ))}
      </>
  ...
  return (
    <div
      ...
      style={{ height: 220 + 30 * requestToShow.length }}
    >
      <div>{message}</div>
      <table>
        <thead>
            ...
            <tr key={index}>
              <td>
                {typeof request.address === "object"
                  ? renderArray(request.address)
                  : request.address}
              </td>
              <td>
                {typeof request.datacap === "object"
                  ? renderArray(request.datacap)
                  : request.datacap}
              </td>
            </tr>
          ))}
      ...
      <div className="ledgermessage">
        Please check your Ledger to sign and send the message.
        <div>
          {" "}
          <ButtonPrimary onClick={props.onClick}>Accept</ButtonPrimary>
        </div>
      </div>
    </div>
  )
