        ...
        <div className='warnmodalledger'>
                ...
                <div className='message' style={{ marginTop: '12px' }}>
                    {!isVerifyWalletLoading ? (
                        ...
                        <div>
                            ...
                            app makes you sign this again, please open a thread{' '}
                            <a
                                href='https://filecoinproject.slack.com/archives/G01HRNU4VBK'
                                target='_blank'
                                rel='noopener noreferrer'
                            >
                                here
                            </a>{' '}
                            pasting the address you are using to verify the wallet.
                        </div>
                    )}
                </div>
            ) : (
                <div className='message' style={{ marginTop: '30px' }}>
                    You need to Sign in via Github to be able to approve request.
                </div>
            )}
            ...
            {github.githubLogged ? (
                <div className='ledgermessage'>
                    {' '}
                    {!isVerifyWalletLoading && (
                        <span>Please check your Ledger to sign and send the message.</span>
                    )}
                    {!isVerifyWalletLoading ? (
                        <ButtonPrimary
                            style={{ marginTop: '12px', minWidth: '130px' }}
                            onClick={() => onClick()}
                        >
                            Verify Wallet
                        </ButtonPrimary>
                    ) : (
                        <ButtonPrimary style={{ marginTop: '12px', minWidth: '130px' }}>
