...
const Notary = (props: { notaryProps: NotaryProps }) => {
    ...
    const [cancelProposalData, setCancelProposalData] = useState<CancelProposalDataType | null>(
        null,
    )
    ...
    const [isApproveLargeRequestModalOpen, setIsApproveLargeRequestModalOpen] = useState(false)

    const openApproveLargeRequestModal = () => setIsApproveLargeRequestModalOpen(true)
    const closeApproveLargeRequestModal = () => setIsApproveLargeRequestModalOpen(false)
    ...
    const showWarnVerify = async (origin: string) => {
        dispatchCustomEvent({
            ...
            detail: {
                ...
                modal: (
                    <WarnModalVerify
                        clientRequests={origin === 'Notary' ? context.clientRequests : []}
                        selectedClientRequests={origin === 'Notary' ? selectedClientRequests : []}
                        onClick={() => {
                            origin === 'Notary'
                                ? verifyClients()
                                : origin === 'newDatacap'
    ...
    const cancelDuplicateRequest = async () => {
        ...
        try {
            ...
            const parsedBody: ProposedRequestBody = ldnParser.parseApprovedRequestWithSignerAddress(
    ...
    const verifyLargeClients = async (i?: LargeRequestData[]) => {
        setApproveLoading(true)
        const thisStateLargeRequestList = Array.isArray(i) ? i : context.largeClientRequests
        console.log('thisStateLargeRequestList', thisStateLargeRequestList)
        for (const request of thisStateLargeRequestList) {
            try {
                ...
                let messageID
                const signer = context.wallet.activeAccount ? context.wallet.activeAccount : ''
                await context.postLogs(
                ...
                setApproveLoading(false)
                // UPDATE THE CONTEXT
                context.updateContextState(thisStateLargeRequestList, 'largeClientRequests')
            } catch (e: any) {
                context.wallet.dispatchNotification('Verification failed: ' + e.message)
                await context.postLogs(
