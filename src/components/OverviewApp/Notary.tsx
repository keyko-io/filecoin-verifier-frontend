...
const Notary = (props: { notaryProps: NotaryProps }) => {
    ...
    const [selectedClientRequests, setSelectedClientRequests] =
        useState([] as any);
    const [
        ...
        if (
            ...
            try {
                const isVerified: any =
                    await context.checkVerifyWallet();
                if (!isVerified) {
                        ...
                        detail: {
                            ...
                            modal: (
                                <WarnModalNotaryVerified
                                    onClick={async () =>
                                        await context.verifyWalletAddress()
                                    }
                                />
                            ),
                        },
    ...
    const cancelDuplicateRequest = async () => {
        ...
        try {
            ...
            const cancelRequestBody = (
                proposedCommentBody: ProposedRequestBody
            ) => {
            ...
            const updateComment =
                context.github.githubOcto.rest.issues.updateComment({
            ...
            const updateCancelData = (item: any) =>
                item.clientAddress !==
                cancelProposalData.clientAddress;

            setDataCancel((dataCancel: any) =>
                dataCancel.filter(updateCancelData)
            );
        ...
        for (const request of context.clientRequests) {
            if (selectedClientRequests.includes(request.number)) {
                ...
                try {
                    ...
                    const datacap: number = anyToBytes(
                        request.data.datacap
                    );
                    console.log("datacap", datacap);
                    address = request.data.address;
                    if (address.length < 12) {
                        address = await context.wallet.api.actorKey(
                            address
                        );
                    }
                    if (context.wallet.multisig) {
                        ...
                        messageID =
                            await context.wallet.api.verifyClient(
                                address,
                    ...
                    const txReceipt =
                        await context.wallet.api.getReceipt(
                            messageID
                        );
                    ...
                    // send notifications
                    context.wallet.dispatchNotification(
                        "Verify Client Message sent with ID: " +
                            messageID
                    );
            ...
            if (
                ...
                try {
                    ...
                    if (request.approvals) {
                        messageID =
                            await context.wallet.api.approvePending(
                                request.multisig,
                                request.tx,
                                context.wallet.walletIndex
                            );
